...
export default function App() {
  ...
  // Fetch recent transfers AND swaps (buys/sells)
  const fetchTransfers = useCallback(async () => {
    ...
    try {
      // Fetch both transfers and swaps in parallel
      const [transfersData, swapsData] = await Promise.all([
        fetchFromAPI(
          `/erc20/${selectedToken.address}/transfers?chain=${selectedToken.chain}&limit=50&order=DESC`
        ).catch(() => ({ result: [] })),
        fetchFromAPI(
          `/erc20/${selectedToken.address}/swaps?chain=${selectedToken.chain}&limit=50`
        ).catch(() => ({ result: [] }))
      ]);
      
      const transfers = (transfersData.result || []).map(t => ({
        ...t,
        activity_type: 'transfer'
      }));
      
      const swaps = (swapsData.result || []).map(s => ({
        from_address: s.from_wallet || s.from_address,
        to_address: s.to_wallet || s.to_address,
        value: s.amount || s.value || s.sold?.amount || '0',
        transaction_hash: s.transaction_hash || s.transactionHash,
        block_timestamp: s.block_timestamp || s.blockTimestamp,
        token_decimals: s.token_decimals || s.sold?.decimals || 18,
        activity_type: 'swap',
        swap_type: s.transaction_type || (s.sold ? 'sell' : 'buy'),
        bought: s.bought,
        sold: s.sold
      }));
      
      // Combine and sort by timestamp
      const combined = [...transfers, ...swaps].sort((a, b) => {
      ...
      console.error('Transfers/swaps fetch error:', err);
      return [];
    }
  }, [selectedToken]);
  ...
  const processedTransfers = React.useMemo(() => {
    return transfers.map(t => {
      ...
      let usdValue = amount * tokenPrice;
      
      // Determine transaction type
      let type;
      if (t.activity_type === 'swap') {
        // It's a swap - determine if buy or sell
        if (t.swap_type === 'sell' || t.sold) {
          type = 'sell';
          // For sells, try to get the amount from sold data
          if (t.sold?.amount) {
            amount = parseFloat(t.sold.amount) / Math.pow(10, t.sold.decimals || decimals);
            usdValue = parseFloat(t.sold.usd_amount || 0) || (amount * tokenPrice);
          }
        } else {
          type = 'buy';
          // For buys, try to get the amount from bought data
          if (t.bought?.amount) {
            amount = parseFloat(t.bought.amount) / Math.pow(10, t.bought.decimals || decimals);
            usdValue = parseFloat(t.bought.usd_amount || 0) || (amount * tokenPrice);
          }
        }
    ...
    .filter(t => t.usdValue >= txMinAmount)
