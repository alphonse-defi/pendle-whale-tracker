...
export default function App() {
  ...
  const fetchHolders = useCallback(async () => {
    ...
    // Return empty and rely on other data sources
    if (selectedToken.chain === 'solana') {
  ...
  const fetchWalletData = useCallback(async (walletAddress, chainOverride = null) => {
    if (!walletAddress) return null;
    
    setWalletLoading(true);
    
    try {
      const chain = chainOverride || selectedToken?.chain || walletViewChain || 'eth';
      
      // Solana wallet data - Moralis Solana API may not work, return basic info
      if (chain === 'solana') {
  ...
  useEffect(() => {
    if (selectedWallet) {
      fetchWalletData(selectedWallet, walletViewChain).then(setWalletData);
    } else {
      setWalletData(null);
    }
  }, [selectedWallet, walletViewChain, fetchWalletData]);
