...
const AddressDisplay = ({ address, chain, label, color = '#00d4ff', badges = [], clickable = true, onClick }) => {
...
const TokenMetricsCard = ({ tokenInfo, holders, poolsData }) => {
  ...
  const totalDexVolume = poolsData?.reduce((sum, p) => sum + (p.volume_24h || 0), 0) || 0;
    ...
    <div style={{ marginBottom: '24px' }}>
      ...
      {isMultiChain && (
        <div style={{ 
          ...
          marginBottom: '12px',
          padding: '10px 14px',
          background: 'linear-gradient(135deg, rgba(123, 47, 247, 0.15) 0%, rgba(241, 7, 163, 0.15) 100%)',
          border: '1px solid rgba(123, 47, 247, 0.3)',
          borderRadius: '10px'
        }}>
          <span style={{ fontSize: '1rem' }}>🌐</span>
          <span style={{ fontSize: '0.8rem', color: '#fff' }}>
            <strong>Multi-Chain Token</strong> — Data aggregated from {chainsWithPools} networks
          </span>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '6px' }}>
            {poolsData?.chainsFetched?.map(chain => (
              <span key={chain} style={{
                background: `${NETWORKS[chain]?.color || '#666'}30`,
                color: NETWORKS[chain]?.color || '#888',
                padding: '3px 8px',
                borderRadius: '4px',
                fontSize: '0.65rem',
                fontWeight: '600'
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px' }}>
        {metrics.map(({ label, value, highlight }) => (
          <div key={label} style={{ 
            background: highlight ? 'rgba(123, 47, 247, 0.1)' : 'rgba(255,255,255,0.02)', 
            border: `1px solid ${highlight ? 'rgba(123, 47, 247, 0.3)' : 'rgba(255,255,255,0.08)'}`, 
            borderRadius: '12px', 
            padding: '16px' 
          }}>
            <div style={{ fontSize: '0.7rem', color: highlight ? '#a855f7' : '#888', marginBottom: '4px', textTransform: 'uppercase' }}>{label}</div>
...
const HolderDistributionChart = ({ holders, tokenInfo }) => {
  ...
  // For multi-chain, percentages may exceed 100% due to same holder on multiple chains
  // We need to normalize to 100% for display
  ...
  // If total exceeds 100%, normalize (this happens with multi-chain aggregation)
  ...
  const segments = [
    { label: 'Top 10 Holders', percent: top10Total, color: '#7b2ff7' },
    { label: 'Top 11-25', percent: Math.max(0, top25Total - top10Total), color: '#f107a3' },
    { label: 'Top 26-50', percent: Math.max(0, top50Total - top25Total), color: '#00d4ff' },
    { label: 'Top 51-100', percent: Math.max(0, top100Total - top50Total), color: '#10b981' },
    { label: 'All Others', percent: othersPercent, color: '#333' }
  ].filter(s => s.percent > 0.1);

  let cumulative = 0;
  const circumference = 2 * Math.PI * 70;

  // Concentration analysis (use normalized values)
  ...
  const concentrationColor = top10Total > 70 ? '#ef4444' : top10Total > 50 ? '#f97316' : top10Total > 30 ? '#eab308' : '#10b981';
  const wasNormalized = normalizationFactor < 1;
    ...
    <div style={{ 
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <h3 style={{ margin: 0, fontSize: '1.1rem', color: '#fff' }}>Holder Distribution</h3>
          {isMultiChain && (
            <span style={{ 
              fontSize: '0.65rem', 
        ...
        <div style={{
          background: `${concentrationColor}20`,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '40px', flexWrap: 'wrap', justifyContent: 'center' }}>
        ...
        <div style={{ position: 'relative', width: '200px', height: '200px' }}>
          ...
          <div style={{ 
            ...
            <div style={{ fontSize: '2rem', fontWeight: '700', color: '#fff' }}>{top10Total.toFixed(1)}%</div>
            <div style={{ fontSize: '0.75rem', color: '#888' }}>Top 10 Holders</div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, minWidth: '200px' }}>
          {segments.map((segment, i) => (
            <div key={i} style={{ 
              ...
              <span style={{ fontSize: '0.85rem', color: '#ccc', flex: 1 }}>{segment.label}</span>
              <span style={{ 
                fontSize: '0.9rem', 
                fontWeight: '700', 
                color: '#fff',
          ...
          <div style={{ 
            ...
            <strong style={{ color: '#fff' }}>Note:</strong> Distribution based on top {holders.length} holders. 
            {isHighlyConcentrated && ' ⚠️ High concentration may indicate whale risk.'}
          </div>
          ...
          <div style={{ 
            ...
            🔜 <strong>Coming Soon:</strong> Complete holder distribution with all token holders, pending increased API support.
          </div>
        </div>
      </div>
    </div>
...
const PoolsTable = ({ pools }) => {
  ...
  // Estimate buy/sell volumes (assume roughly proportional to transaction counts)
  // This is an approximation since GeckoTerminal doesn't give exact buy/sell USD volumes
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '20px', marginBottom: '24px' }}>
      <h3 style={{ margin: '0 0 16px 0', fontSize: '1rem', color: '#fff' }}>Liquidity & Activity</h3>
      
      {/* Summary Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '20px' }}>
        <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '10px', padding: '12px', textAlign: 'center' }}>
          <div style={{ fontSize: '0.65rem', color: '#888', marginBottom: '4px' }}>Total Liquidity</div>
          <div style={{ fontSize: '1.1rem', fontWeight: '700', color: '#fff' }}>{formatNum(totalLiquidity)}</div>
        </div>
        <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '10px', padding: '12px', textAlign: 'center' }}>
          <div style={{ fontSize: '0.65rem', color: '#888', marginBottom: '4px' }}>24h Volume</div>
          <div style={{ fontSize: '1.1rem', fontWeight: '700', color: '#fff' }}>{formatNum(totalVolume)}</div>
        </div>
        <div style={{ background: 'rgba(16, 185, 129, 0.1)', borderRadius: '10px', padding: '12px', textAlign: 'center' }}>
          <div style={{ fontSize: '0.65rem', color: '#10b981', marginBottom: '4px' }}>24h Buy Volume</div>
          <div style={{ fontSize: '1.1rem', fontWeight: '700', color: '#10b981' }}>{formatNum(buyVolume)}</div>
          <div style={{ fontSize: '0.6rem', color: '#888' }}>{totalBuys.toLocaleString()} txns</div>
        </div>
        <div style={{ background: 'rgba(239, 68, 68, 0.1)', borderRadius: '10px', padding: '12px', textAlign: 'center' }}>
          <div style={{ fontSize: '0.65rem', color: '#ef4444', marginBottom: '4px' }}>24h Sell Volume</div>
          <div style={{ fontSize: '1.1rem', fontWeight: '700', color: '#ef4444' }}>{formatNum(sellVolume)}</div>
          <div style={{ fontSize: '0.6rem', color: '#888' }}>{totalSells.toLocaleString()} txns</div>
        </div>
      </div>

      {/* Buy/Sell Ratio Bar */}
      <div style={{ marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '0.7rem', color: '#888' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px', fontSize: '0.7rem' }}>
          <span style={{ color: '#10b981' }}>{totalTxs > 0 ? (totalBuys / totalTxs * 100).toFixed(1) : 50}%</span>
          <span style={{ color: '#ef4444' }}>{totalTxs > 0 ? (totalSells / totalTxs * 100).toFixed(1) : 50}%</span>
        </div>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <th style={{ padding: '10px', textAlign: 'right', color: '#888' }}>Buys</th>
              <th style={{ padding: '10px', textAlign: 'right', color: '#888' }}>Sells</th>
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <td style={{ padding: '12px 10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    {pool.chain && (
                      <span style={{
                        background: `${NETWORKS[pool.chain]?.color || '#666'}30`,
                        color: NETWORKS[pool.chain]?.color || '#888',
                        padding: '2px 6px',
                        borderRadius: '4px',
                        fontSize: '0.6rem',
                        fontWeight: '600'
                ...
                <td style={{ padding: '12px 10px', textAlign: 'right', color: '#10b981', fontWeight: '600' }}>{(pool.buys_24h || 0).toLocaleString()}</td>
                <td style={{ padding: '12px 10px', textAlign: 'right', color: '#ef4444', fontWeight: '600' }}>{(pool.sells_24h || 0).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
const WalletProfiler = ({ wallet, walletData, loading, onClose, chain }) => {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
      <div style={{ background: 'linear-gradient(180deg, #1a1a2e 0%, #0f0f1a 100%)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', width: '100%', maxWidth: '800px', maxHeight: '85vh', overflow: 'auto' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: '#1a1a2e', zIndex: 1 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: '1.2rem', color: '#fff' }}>
              🔍 Wallet Profile {isMultiChain && <span style={{ fontSize: '0.7rem', background: 'linear-gradient(135deg, #7b2ff7 0%, #f107a3 100%)', padding: '2px 8px', borderRadius: '4px', marginLeft: '8px' }}>Multi-Chain</span>}
            </h2>
        ...
        <div style={{ padding: '24px' }}>
          {loading ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#888' }}><LoadingSpinner size={30} /><div style={{ marginTop: '16px' }}>Loading across all chains...</div></div>
          ) : walletData ? (
            <>
              {/* Summary Stats */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '24px' }}>
                <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '12px', padding: '16px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#10b981' }}>{formatUSD(totalValue)}</div>
                </div>
                <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '12px', padding: '16px', textAlign: 'center' }}>
                  <div style={{ fontSize: '0.7rem', color: '#888' }}>Unique Tokens</div>
                  <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#fff' }}>{walletData.token_count || 0}</div>
                </div>
                <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '12px', padding: '16px', textAlign: 'center' }}>
                  <div style={{ fontSize: '0.7rem', color: '#888' }}>Chains Active</div>
                  <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#a855f7' }}>{chainBreakdown.length || 1}</div>
                </div>
              </div>
              ...
              {isMultiChain && chainBreakdown.length > 0 && (
                <div style={{ marginBottom: '24px' }}>
                  <h3 style={{ margin: '0 0 12px 0', fontSize: '0.9rem', color: '#fff' }}>Value by Chain</h3>
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    {chainBreakdown.sort((a, b) => b.value - a.value).map((c, i) => {
                      const chainNet = NETWORKS[c.chain];
                      const pct = totalValue > 0 ? (c.value / totalValue * 100) : 0;
                      return (
                        <div key={i} style={{
                          background: `${chainNet?.color || '#666'}15`,
                          border: `1px solid ${chainNet?.color || '#666'}40`,
                          borderRadius: '8px',
                          padding: '10px 14px',
                          minWidth: '120px'
                        }}>
                          <div style={{ fontSize: '0.7rem', color: chainNet?.color || '#888', fontWeight: '600' }}>{c.name}</div>
                          <div style={{ fontSize: '1rem', fontWeight: '700', color: '#fff' }}>{formatUSD(c.value)}</div>
                ...
                {topHoldings.length > 0 ? (
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                    <thead><tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                      ...
                      {isMultiChain && <th style={{ padding: '12px', textAlign: 'left', color: '#888' }}>Chains</th>}
                        ...
                        <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                          <td style={{ padding: '12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              ...
                              <div><div style={{ fontWeight: '600', color: '#fff' }}>{t.symbol}</div><div style={{ fontSize: '0.7rem', color: '#666' }}>{t.name?.slice(0, 20)}</div></div>
                            </div>
                          </td>
                          {isMultiChain && (
                            <td style={{ padding: '12px' }}>
  ...
  return (
    <div style={{ marginTop: '24px' }}>
      ...
      <div style={{
        ...
        <h2 style={{ margin: '0 0 16px 0', fontSize: '1.2rem', color: '#fff' }}>
          👛 Track Wallets
        </h2>
        <p style={{ color: '#888', fontSize: '0.85rem', marginBottom: '16px' }}>
          Add wallet addresses to track their holdings and activity across chains.
        </p>
      ...
      {trackedWallets.length > 0 && (
        <div style={{
          ...
          <h3 style={{ margin: '0 0 16px 0', fontSize: '1rem', color: '#fff' }}>
            Your Tracked Wallets ({trackedWallets.length})
          </h3>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ margin: 0, color: THEME.text.secondary, fontSize: '0.875rem' }}>
              Track whale movements across tokens and networks
            </p>
          </div>
