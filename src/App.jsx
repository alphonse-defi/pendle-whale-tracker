  ...
  return (
    <div style={{
      ...
      <div style={{ 
        ...
        <div>
          <h3 style={{ margin: 0, fontSize: '0.9375rem', fontWeight: '600', color: THEME.text.primary }}>Price</h3>
          <div style={{ fontSize: '1.25rem', fontWeight: '600', color: THEME.text.primary, marginTop: '4px' }}>
            ${currentPrice < 0.01 ? currentPrice.toFixed(8) : currentPrice.toFixed(2)}
            <span style={{ fontSize: '0.8rem', marginLeft: '8px', color: isPositive ? THEME.accent.success : THEME.accent.error }}>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {['7', '30', '90'].map(days => (
            <button
              ...
              style={{
                padding: '5px 10px',
                background: timeframe === days ? THEME.accent.primary : THEME.bg.tertiary,
        ...
        <svg width="100%" height="100%" viewBox="0 0 400 120" preserveAspectRatio="none">
          <defs>
            <linearGradient id="chartGradient" x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%" stopColor={isPositive ? THEME.accent.success : THEME.accent.error} stopOpacity="0.2" />
              <stop offset="100%" stopColor={isPositive ? THEME.accent.success : THEME.accent.error} stopOpacity="0" />
            </linearGradient>
          </defs>
          <path
            d={`M 0 ${120 - ((chartData[0]?.price - minPrice) / priceRange) * 110} ` +
              chartData.map((d, i) => `L ${(i / (chartData.length - 1)) * 400} ${120 - ((d.price - minPrice) / priceRange) * 110}`).join(' ') +
              ` L 400 120 L 0 120 Z`}
          ...
          <path
            d={`M 0 ${120 - ((chartData[0]?.price - minPrice) / priceRange) * 110} ` +
              chartData.map((d, i) => `L ${(i / (chartData.length - 1)) * 400} ${120 - ((d.price - minPrice) / priceRange) * 110}`).join(' ')}
            fill="none"
            stroke={isPositive ? THEME.accent.success : THEME.accent.error}
            strokeWidth="1.5"
          />
        </svg>
        <div style={{ position: 'absolute', right: 0, top: 0, fontSize: '0.65rem', color: THEME.text.muted }}>
