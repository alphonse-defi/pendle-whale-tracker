...
const TransactionFilters = ({ minAmount, setMinAmount, txType, setTxType }) => (
  <div style={{
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      >
        <option value={0}>Auto (Top 25%)</option>
...
export default function App() {
  ...
  const fetchTransfers = useCallback(async () => {
    ...
    try {
      // For EVM chains, fetch transfers and try to get DEX trades
      // The /erc20/{address}/transfers endpoint gets all token transfers
      // We'll fetch more and identify DEX swaps by analyzing the addresses
      
      const transfersData = await fetchFromAPI(
        `/erc20/${selectedToken.address}/transfers?chain=${selectedToken.chain}&limit=200&order=DESC`
      ).catch(() => ({ result: [] }));
      
      // Map transfers and try to identify swaps (transfers involving DEX routers)
  ...
  const processedTransfers = React.useMemo(() => {
    ...
    // Calculate dynamic threshold (show top transactions)
    // If no manual filter set, show transactions above the 75th percentile
    let filtered = withUsdValues;
    
    if (txMinAmount === 0 && withUsdValues.length > 10) {
      // Sort by USD value to find percentile threshold
      const sortedByValue = [...withUsdValues].sort((a, b) => b.usdValue - a.usdValue);
      const percentileIndex = Math.floor(sortedByValue.length * 0.25); // Top 25%
      const dynamicThreshold = sortedByValue[percentileIndex]?.usdValue || 0;
      
      // Apply dynamic threshold (minimum $100 to avoid noise)
      const threshold = Math.max(dynamicThreshold, 100);
      filtered = withUsdValues.filter(t => t.usdValue >= threshold);
    } else if (txMinAmount > 0) {
      filtered = withUsdValues.filter(t => t.usdValue >= txMinAmount);
    }
    ...
    return filtered.sort((a, b) => b.usdValue - a.usdValue);
